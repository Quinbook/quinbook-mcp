...
const SERVER_INFO = { name: 'quinbook-mcp', version: '0.2.9' } as const;
...
/**
 * Build a fully-wired McpServer (instructions + all tools registered) around a
 * given ApiClient/token manager. Transport-agnostic: the stdio entry point
 * (index.ts) and the remote HTTP entry point (server-http.ts) both call this,
 * so tool behaviour is identical across the local and remote connectors.
 */
export function createMcpServer(api: ApiClient, tokens: ITokenManager): McpServer {
    ...
    server.registerTool(
      ...
      async (rawInput: unknown) => {
        try {
          ...
          const message =
            e instanceof ApiError ? `API error ${err.message}` : `${err.name}: ${err.message}`;
          return {
            isError: true,
